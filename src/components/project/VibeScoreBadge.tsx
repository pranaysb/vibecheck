...
export function VibeScoreBadge({ score, size = "md", showLabel = false }: VibeScoreBadgeProps) {
  ...
  if (size === "hero") {
    return (
      <div className="flex items-baseline gap-2 group">
        <div className={`text-6xl sm:text-7xl font-mono font-black tracking-tight ${colors.text} drop-shadow-sm`}>
  ...
  if (size === "lg") {
    return (
      <div className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border font-mono font-bold shadow-sm ${colors.badge}`}>
  ...
  // Default md
  return (
    <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-mono font-bold border shadow-sm ${colors.badge}`}>
      <NumberTicker value={score} />
